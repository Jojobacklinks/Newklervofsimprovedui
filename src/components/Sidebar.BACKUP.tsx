...
import { Plus, ChevronDown, LayoutDashboard, Users, Calendar, Briefcase, ClipboardList, FolderOpen, PackageOpen, MessageSquare, HelpCircle, Menu, X, Settings, ChevronLeft, ChevronRight, Zap, HardHat, Sparkles, ChartPie, BadgeCheck } from 'lucide-react';
...
const menuItems = [
  ...
  { icon: BadgeCheck, label: 'Service Plans', path: '/service-plans' },
  { 
    icon: Briefcase, 
    label: 'Jobs', 
    path: '/jobs',
    subItems: [
      { label: 'All Jobs', path: '/jobs/all-jobs' },
      { label: 'Estimates', path: '/jobs/estimates' },
      { label: 'Invoices', path: '/jobs/invoices' }
    ]
  },
  { icon: Users, label: 'Clients', path: '/clients' },
  { icon: ChartPie, label: 'Reports', path: '/reports' },
  { icon: PackageOpen, label: 'Inventory', path: '/inventory' },
  { icon: HardHat, label: 'Team', path: '/team' },
  { icon: Sparkles, label: 'Klervo AI', path: '/klervo-ai' },
];

const addNewActions = [
  { label: '+ Add Job', action: 'add-job' },
  { label: '+ Add Lead', action: 'add-lead' },
  { label: '+ Add Team', action: 'add-team' },
  { label: '+ Add Plan', action: 'add-plan' },
  { label: '+ Add Client', action: 'add-client' },
];
    ...
    <>
      ...
      >
        ...
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <Link to="/" className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-1'}`} onClick={handleLinkClick}>
            <img 
              src={logoImage} 
              alt="KLERVO" 
